...
<div id="content-wrapper">
    ...
    <div class="page-header">			
        <div class="row">
            <h1 class="col-xs-12 col-sm-4 text-center text-left-sm"><i class="fa fa-dashboard page-header-icon"></i>&nbsp;&nbsp;Grade </h1>
            <div class="col-xs-12 col-sm-8">
                <div class="row">                    
                    <hr class="visible-xs no-grid-gutter-h">
                    <div class="pull-right col-xs-12 col-sm-auto"><a style="width: 100%;" class="btn btn-primary btn-labeled" href="<?=base_url($currentModule."/add_grade")?>"><span class="btn-label icon fa fa-plus"></span>Add Grade</a></div>                        
        ...
        <div class="row ">
			...
			?>
                <div class="panel-heading">
                        <span class="panel-title">Grade</span>
                ...
                <div class="panel-body">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody id="itemContainer">
                            <?php
                            $j=1;
                            for($i=0;$i<count($grade_details);$i++)
                            {
                            ?>
                            <tr <?=$grade_details[$i]["status"]=="N"?"style='background-color:#FBEFF2'":""?>>
                                <td><?=$j?></td>
                                <td><?=$grade_details[$i]['grade_level']?></td>
                           <td><?=$grade_details[$i]['grade_marks']?></td>
                              <td>
                                    <a href="<?=base_url($currentModule."/edit_grade/".$grade_details[$i]['grade_id'])?>"><i class="fa fa-edit"></i></a>                                                                        
                    <a href='<?=$grade_details[$i]["status"]=="Y"?"disable/".$grade_details[$i]["subject_id"]:"enable/".$grade_details[$i]["grade_id"]?>'><i class='fa <?=$grade_details[$i]["status"]=="Y"?"fa-ban":"fa-check"?>' title='<?=$grade_details[$i]["status"]=="Y"?"Disable":"Enable"?>'></i></a>
                                </td>
                            </tr>
                            <?php
                            $j++;
